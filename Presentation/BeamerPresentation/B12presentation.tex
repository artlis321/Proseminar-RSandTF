...
\begin{frame}{Kronecker function}{\tiny \cite{BL13} Section 3.4}
    \begin{block}{Definitions of the Kronecker function}
        ...
        \begin{enumerate}
            ...
            \[-2\pi i \left(\frac{z}{1-z} + \frac{1}{1-w} + \sum_{m,n > 0} (z^m w^n - z^{-m} w^{-n}) q^{mn}\right) \quad , \quad \begin{pmatrix} z \\ w \\ q \end{pmatrix} = \ee \begin{pmatrix}z \\ \alpha \\ \tau\end{pmatrix}\]
...
\begin{frame}{Differentials from the Kronecker function}
    \[\alpha F(z,\alpha) dz = \sum_{n=0}^\infty g^{(n)}(z) \alpha^n \]
    \vspace{+1em}
    \[g^{(0)}(z) = 1\]
...
\begin{frame}{Fay relation for differentials}
    \[F(z_1,\alpha_1)F(z_2,\alpha_2) = F(z_1,\alpha_1+\alpha_2)F(z_2-z_1,\alpha_2)+F(z_2,\alpha_1+\alpha_2)F(z_1-z_2,\alpha_1)\]
    \begin{align*}
        \left(\sum_{n=0}^\infty g^{(n)}(z_1) \alpha_1^n\right)\left(\sum_{n=0}^\infty g^{(n)}(z_2) \alpha_2^n\right) = 
        \left(\sum_{n=0}^\infty g^{(n)}(z_1) (\alpha_1+\alpha_2)^n\right)\left(\sum_{n=0}^\infty g^{(n)}(z_2-z_1) \alpha_2^n\right) + \\
        \left(\sum_{n=0}^\infty g^{(n)}(z_2) (\alpha_1+\alpha_2)^n\right)\left(\sum_{n=0}^\infty g^{(n)}(z_1-z_2) \alpha_1^n\right)
    \end{align*}
    \[\downarrow \text{match powers and use }g^{(0)}=1\]
    \begin{align*}
        g^{(m)}(z_1)g^{(n)}(z_2) = (-1)^{n+1}g^{(m+n)}(z_1-z_2) + \\
        + \sum_{r=0}^n \begin{pmatrix} m+r-1 \\ m-1 \end{pmatrix} g^{(n-r)}(z_2-z_1) g^{(m+r)}(z_1) \\
        + \sum_{r=0}^m \begin{pmatrix} n+r-1 \\ n-1 \end{pmatrix} g^{(m-r)}(z_1-z_2) g^{(n+r)}(z_2) 
    \end{align*}
\end{frame}
