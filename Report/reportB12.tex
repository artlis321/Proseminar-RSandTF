...
\begin{definition}[Holomorphic functions from the Kronecker function]
    ...
    \end{equation}
    we find the functions $g^{(n)}(z)$, which are suitable integration kernels for elliptic polylogarithms (\ref{rep:B11}) \cite{Broedel_2022}.
\end{definition}

The resulting differentials have power expansions in $q = \bolde(\tau)$ \cite{Broedel_2015}
\begin{equation}
    g^{(0)}(z) = 1,
\end{equation}
\begin{equation}
